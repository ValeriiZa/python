...
def selection (arr):
    ...
    for i in range(0, arr_size):
        ...
        if min != i:
            ...
            arr[min] = temp
        #print (arr)
    print ("Selection sort time:", datetime.now()-t)
...
def insertion (arr):
    ...
    for i in range (1, arr_size):
        ...
        while ( arr[j]>arr[j+1] and j>=0 ):
            ...
            j=j-1
        print (arr)
    print ("Insertion sort time:", datetime.now()-t)
...
#    as a single element is already sorted.
    if ( len(l) > 1 ) :
        sort (arr, start, start+len(l)-1)
    if ( len(r) > 1 ):
        sort (arr, start+len(l)+1, start+len(l)+len(r))
...
print ("Arr after using SORT method: ", sort (arr1) )
#bubble_asc  (arr1)
#bubble_desc (arr1)
